...
* 콘텐츠를 효율적으로 전달하기 위해 분산된 서버에 데이터를 저장해 사용자에게 전달하는 시스템이다.
  * 데이터 요청이 왔을 때 요청 위치로부터 가장 가까운 서버에서 데이터를 전송해준다.
* 예) 자바스크립트 라이브러리가 필요할 때 외부 cdn으로부터 제공받는 방식을 사용할 수 있다.
  * 단 외부 서비스에 문제가 생기면 우리 서비스에도 문제가 생기기 때문에 실제 서비스에서는 잘 사용하지 않는다.
* 실제 서비스에서는 주로 직접 라이브러리를 다운 받아서 사용한다.
  * npm/bower/yarn + grunt/gulp/webpack 등을 통한 방법이 있다.

참고 자료 : https://study-ihl.tistory.com/38

<br>

#### @Query 란?

* Spring Data JPA에 속하는 애노테이션으로 JPQL을 통해 복잡한 쿼리를 직접 짤 수 있다.
* JPQL : 테이블이 아닌 엔티티 객체를 대상으로 검색하는 객체지향 쿼리, SQL 추상화로 인해 특정 db sql에 의존하지 않는다.
...
#### 데이터 조회용 프레임워크 (책 본문에 있는 내용)

* 규모가 있는 프로젝트에서의 데이터 조회는 FK의 조인, 복잡한 조건 등으로 인해 Entity 클래스만으로 처리하기 어려워 조회용 프레임워크를 추가로 사용합니다.
* 예) querydsl, joop, MyBatis
* 저자는 이 중에 querydsl을 추천하였습니다. 그 이유는 타입 안정성이 보장되고, 국내 많은 회사에서 사용 중이며, 레퍼런스가 많다는 점이 있습니다.

<br>

#### 인상 깊은 구절

* 웹 요청에서의 테스트 코드 작성 방법은 필수로 익혀가길 바랍니다.
